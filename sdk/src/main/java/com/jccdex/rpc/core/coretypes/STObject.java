...
import com.jccdex.rpc.core.fields.AmountField;
import com.jccdex.rpc.core.fields.BlobField;
import com.jccdex.rpc.core.fields.Field;
import com.jccdex.rpc.core.fields.HasField;
...
import com.jccdex.rpc.core.fields.STObjectField;
import com.jccdex.rpc.core.fields.Type;
import com.jccdex.rpc.core.fields.UInt16Field;
import com.jccdex.rpc.core.fields.UInt32Field;
import com.jccdex.rpc.core.fields.UInt64Field;
import com.jccdex.rpc.core.fields.UInt8Field;
...
public class STObject implements SerializedType, Iterable<Field> {
	...
	protected FieldsMap fields;// 用户最终传入的所有参数
	...
	public void checkFormat() {
		...
		for (Field field : this) {
			if (!requirements.containsKey(field)) {
				throw new FormatException(fmt.name() + " doesn't have field: " + field);
			}
		}
		for (Field field : requirements.keySet()) {
			...
			if (!has(field)) {
				if (req == Format.Requirement.REQUIRED) {
					throw new FormatException(fmt.name() + " requires " + field + " of type " + field.getType());
				}
			} else {
				SerializedType type = get(field);
				if (type.type() != field.getType()) {
					if (!(field.getType() == Type.Hash160 && type.type() == Type.AccountID)) {
						throw new FormatException(type.toString() + " is not " + field.getType());
					}
				}
			}
		}
